...
namespace Playbook.Bussiness.Model
{
    ...
    /// </summary>
    public partial class PlaybookFactory : PlaybookFactoryBase
    {
        public async Task<List<Playbook>> FindAllActiveAsync(SortOrder sort_order, string sort_column, 
            string search_term, int skip, int limit)
        {
            ...
            string sortColumn = TransformNameColumn<Playbook>(sort_column);
        ...
        public async Task<Playbook?> FindAsync(String id)
        {
            await using var dataHandler = new DatabaseHandler();
